...
def quantize_weight_groupwise_nbit(
    ...
    """
    Group-wise symmetric quantization along input dim (axis=1), per output row.
    For bits > 4 or group_size<=0, falls back to per-channel along axis=1.
    Returns (w_deq: float32[ out, in ], info).
    """
...
class ActFakeQuantPerChannelNbit(nn.Module):
    """Per-channel (last-dim) symmetric n-bit fake-quant using a fixed scale vector [C]."""
    def __init__(self, a_bits: int, a_scale_vec: torch.Tensor, eps: float = 1e-8):
        ...
        self.register_buffer("a_scale_vec", a_scale_vec, persistent=False)  # [C]
...
class ActFakeQuantPerChannelQAT(nn.Module):
    ...
    Per-channel (last-dim) n-bit fake-quant for QAT:
      - STE (straight-through) quantization so grads flow to inputs/LoRA.
      - EMA update of per-channel scales with robust percentile.
    """
    def __init__(self, a_bits: int, init_scale_vec: torch.Tensor, momentum: float = 0.99,
        ...
        self.register_buffer("a_scale_vec", init_scale_vec, persistent=False)  # [C]
...
class SQQuantLinearNbit(nn.Module):
    ...
    - A: Identity for 8b (robust) or fixed per-(tensor/channel) for <8b.
...
def recalibrate_act_scales_post_sq(model: nn.Module, calib_loader, device, quantile: float | None):
    """
    After swapping in SQQuantLinearNbit modules, re-estimate activation scales on the transformed model.
    Robust to large tensors (reduced dims + CPU fallback). Skips Identity quantizers.
    """
    model.eval().to(device)
...
class SQQuantLoRALinearW4A4(nn.Module):
    ...
    SmoothQuant + W4A4 fake-quant + trainable LoRA (A,B) adapters.
      - Base weights quantized to 4-bit (group-wise); stored dequantized (float) in w_deq and frozen.
      - Activations quantized to A4 with per-channel scales (QAT: EMA + STE).
      - LoRA path is trained in FP (small rank), applied on the quantized activations.
    """
    def __init__(
        self,
        lin: nn.Linear,
        ...
        *,
        ...
        # SmoothQuant scale (input channels)
        self.register_buffer("s_vec", s_vec.to(dev, dtype=torch.float32), persistent=False)   # [in]

        # Bias (optionally trainable)
        ...
        # ------- Base 4-bit weights (group-wise) -------
        w = lin.weight.detach().to(torch.float32)                                    # [out, in] on dev
        w_prime = w * (self.s_vec.unsqueeze(0))
        ...
        # ------- Activation A4 (per-channel, QAT: EMA + STE) -------
        ...
        # ------- Bias correction on base path (optional) -------
        if bias_correction and (self.bias is not None) and (mean_vec is not None):
            mean_x_prime = mean_vec.to(dev, dtype=torch.float32) / self.s_vec         # [in]
            delta = (w_prime - w_deq)                                                 # [out, in]
            with torch.no_grad():
    ...
    def forward(self, x: torch.Tensor):
        ...
        # A4 activation fake-quant (QAT: EMA update + STE)
        x_q = self.act_q(x)

        # Base path (quantized weights stored dequantized as float)
        y = torch.matmul(x_q, self.w_deq.t())  # [*, out]

        # LoRA delta: (drop(x_q) @ A) @ B * scaling
        xq_drop = self.lora_dropout(x_q)
        r = torch.matmul(xq_drop, self.lora_A)       # [*, r]
    ...
    def merge_lora_(self):
        """Merge LoRA into base weight buffer for inference."""
...
def apply_smoothquant_with_lora_w4a4(model: nn.Module, act_stat: dict, alpha: float = 0.80,
                                     ...
                                     eps: float = 1e-8):
    """
    Replace every nn.Linear (except lm_head) with SQQuantLoRALinearW4A4.
    Returns list of trainable LoRA parameters (and optional trainable biases).
    """
    trainable = []
    def transform(module: nn.Module):
        nonlocal trainable
...
def recalibrate_act_scales_post_lora_w4a4(model: nn.Module, calib_loader, device, quantile: float | None):
    """
    Re-estimate A4 per-channel scales on the transformed model for SQ+LoRA layers.
    (Mostly redundant with QAT EMA, but helps initialize.)
    """
    model.eval().to(device)
...
def main():
    ...
    if args.mode == "fp16":
        ...
        # Hard-coded low-bit recipe for QAT
        if args.w_group_size == 0: args.w_group_size = 64
        if args.w_clip_q is None: args.w_clip_q = 0.9995
        if args.alpha < 0.75: args.alpha = 0.80  # shift more range to weights for A4 stability

        print(f"Calibrating activations for SmoothQuant (alpha={args.alpha}, q={args.quantile})…")
        model = model.float()
        # Use train split for calib to better match training data stats
